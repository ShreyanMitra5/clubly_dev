...
export async function POST(request: NextRequest) {
  try {
    ...
    const { clubId, clubTopic, semesterStart, semesterEnd, frequency, specialEvents } = body;
    console.log('[generate-topics] clubId:', clubId, 'body:', body);

    if (!GROQ_API_KEY) {
      return NextResponse.json({ error: 'Groq API key not set in environment variables.' }, { status: 500 });
    }
    if (!clubId || !clubTopic || !semesterStart || !semesterEnd || !frequency) {
