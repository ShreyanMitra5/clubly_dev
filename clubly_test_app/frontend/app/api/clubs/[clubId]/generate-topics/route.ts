...
export async function POST(req: Request) {
  try {
    ...
    let prompt = `Generate exactly ${dates.length} specific, actionable, and engaging meeting topics for a ${topic} club. 
    Each topic should be highly relevant, concrete, and use real-world terms. The club name is "${clubName}".
    ...
    - "meetings": an array of exactly ${dates.length} objects, each with EXACTLY these fields:
    ...
    - "specialEvents": an array of 2-3 creative, fun, or community-oriented club events, each with EXACTLY these fields:
    ...
    IMPORTANT: Generate exactly ${dates.length} meeting topics. Do not stop early. Complete all ${dates.length} topics.
    ...
    console.log(`Generating topics for ${dates.length} meetings...`);
    ...
    try {
      ...
      console.log(`Successfully parsed ${meetings.length} meetings and ${specialEvents.length} special events`);
    } catch (error) {
    ...
    // If we don't have enough meetings, generate additional ones
    if (meetings.length < dates.length) {
      console.log(`Generated ${meetings.length} meetings, need ${dates.length}. Generating additional topics...`);
      
      const additionalTopics = [
        { topic: "Advanced ${topic} Concepts", description: "Deep dive into advanced topics and techniques.", prerequisites: "Previous meetings" },
        { topic: "Guest Speaker Session", description: "Learn from industry experts and professionals.", prerequisites: "None" },
        { topic: "Project Showcase", description: "Members present their projects and get feedback.", prerequisites: "Active participation" },
        { topic: "Workshop Day", description: "Hands-on workshop with practical exercises.", prerequisites: "Basic knowledge" },
        { topic: "Q&A and Discussion", description: "Open forum for questions and group discussion.", prerequisites: "None" },
        { topic: "Skill Building Session", description: "Focus on developing specific skills and competencies.", prerequisites: "None" },
        { topic: "Industry Trends", description: "Explore current trends and future directions.", prerequisites: "None" },
        { topic: "Collaboration Workshop", description: "Team-based activities and collaborative learning.", prerequisites: "None" },
        { topic: "Review and Planning", description: "Review progress and plan upcoming activities.", prerequisites: "None" },
        { topic: "Special Topic Session", description: "Covering specialized topics and advanced concepts.", prerequisites: "Previous experience" }
      ];

      // Add additional topics to fill the gap
      for (let i = meetings.length; i < dates.length; i++) {
        const additionalTopic = additionalTopics[i % additionalTopics.length];
        meetings.push({
          topic: additionalTopic.topic.replace('${topic}', topic),
          description: additionalTopic.description,
          prerequisites: additionalTopic.prerequisites
        });
      }
    }
