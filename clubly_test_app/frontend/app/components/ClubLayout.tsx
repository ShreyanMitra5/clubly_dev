...
function EmailPanel({ clubName, clubInfo }: { clubName: string; clubInfo: any }) {
  ...
  const loadContacts = async () => {
    ...
    try {
      ...
      // If no contacts in Supabase, try to load from JSON file and migrate
      if (!data || data.length === 0) {
        try {
          const response = await fetch(`/api/clubs/${clubId}/emails`);
          if (response.ok) {
            const jsonData = await response.json();
            if (jsonData.contacts && jsonData.contacts.length > 0) {
              // Migrate contacts from JSON to Supabase
              const supabaseContacts = jsonData.contacts.map((contact: any) => ({
                club_id: clubId,
                email: contact.email,
          ...
          console.error('Failed to migrate contacts:', migrationError);
        }
      }
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-2 font-light">
                CSV should have columns: email (required), name (optional).<br/>
                Supported email column names: email, e-mail, email address, mail, contact email
              </p>
            </div>
            <button
