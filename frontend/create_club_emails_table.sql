-- Create club_emails table for email contacts
...
-- 3. Add unique constraint to prevent duplicate emails per club (only if it doesn't exist)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'unique_club_email' 
        AND conrelid = 'club_emails'::regclass
    ) THEN
...
DROP TRIGGER IF EXISTS update_club_emails_updated_at ON club_emails;
