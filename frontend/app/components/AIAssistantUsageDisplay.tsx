...
export default function AIAssistantUsageDisplay({
  usageCount,
  limit,
  remainingMessages,
  canSendMessage,
  currentMonth,
  isLoading = false
}: AIAssistantUsageDisplayProps) {
  const [showModal, setShowModal] = useState(false);
  const usagePercentage = (usageCount / limit) * 100;
  const isNearLimit = usageCount >= limit - 10; // Warning when 10 messages left
  ...
  // Only show the modal when at limit, don't render the full component
  if (!isAtLimit) {
    return null;
  }
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-lg font-light text-gray-900">AI Assistant Usage</h3>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        >
          ...
          <div className="text-sm text-gray-600 font-extralight">Sent</div>
        </motion.div>
        ...
        >
          <div className="text-2xl font-light text-gray-900 mb-1">{remainingMessages}</div>
          ...
          >
            <div className="text-center">
              ...
              <p className="text-red-700 font-light text-sm mb-2">
                You've reached your monthly limit of {limit} AI assistant messages.
              </p>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-yellow-700 font-light text-xs">
                  You have {remainingMessages} message{remainingMessages !== 1 ? 's' : ''} left this month
                </p>
              </div>
            </div>
            <div className="text-right">
              <div className="text-yellow-800 font-semibold text-lg">{usageCount}/{limit}</div>
              <div className="text-yellow-600 font-light text-xs">Sent</div>
            </div>
          </div>
        </motion.div>
      )}
      ...
      {!isNearLimit && !isAtLimit && (
        <motion.div
          className="bg-purple-50/50 border border-purple-200/50 rounded-xl p-4"
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-purple-700 font-light text-sm">
                You have {remainingMessages} AI assistant message{remainingMessages !== 1 ? 's' : ''} available this month.
              </p>
              <p className="text-purple-600 font-extralight text-xs mt-1">
                Chat with your AI advisor for club guidance!
              </p>
            </div>
          </div>
        </motion.div>
      ...
      >
        <div className="flex items-center justify-between text-xs text-gray-500 font-extralight">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span>{usageCount}/{limit} sent</span>
          </div>
        </div>
      </motion.div>
    </motion.div>
