...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Checking roadmap usage for clubId:', clubId);
    ...
    console.log('Querying roadmap_usage table...');
    ...
    if (fetchError && fetchError.code !== 'PGRST116') { // PGRST116 = no rows returned
      console.error('Error fetching roadmap usage data:', fetchError);
      return NextResponse.json({ error: 'Failed to fetch usage data' }, { status: 500 });
    }

    console.log('Roadmap usage data:', usageData);
    const currentUsage = usageData?.usage_count || 0;
    const remainingSlots = Math.max(0, monthlyLimit - currentUsage);
    const canGenerate = remainingSlots > 0;
    
    console.log('Current roadmap usage:', currentUsage, 'Remaining slots:', remainingSlots, 'Can generate:', canGenerate);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.log('Updating roadmap usage for clubId:', clubId, 'userId:', userId);
    const currentMonthYear = getCurrentMonthYear();
    console.log('Current month/year:', currentMonthYear);
    ...
    if (fetchError && fetchError.code !== 'PGRST116') { // PGRST116 = no rows returned
      console.error('Error fetching current roadmap usage:', fetchError);
    ...
    if (updateError) {
      console.error('Error updating roadmap usage data:', updateError);
