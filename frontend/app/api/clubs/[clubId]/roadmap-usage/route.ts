...
export async function GET(
  ...
  try {
    ...
    if (usageError) {
      console.error('Error fetching roadmap usage:', usageError);
    ...
    console.error('Roadmap usage GET error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

// POST to record a new roadmap generation
export async function POST(
  ...
  try {
    ...
    if (checkError) {
      console.error('Error checking roadmap usage:', checkError);
    ...
    if (error) {
      console.error('Error recording roadmap usage:', error);
    ...
    console.error('Roadmap usage POST error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
