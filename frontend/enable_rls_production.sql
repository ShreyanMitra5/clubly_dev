...
-- This script enables RLS on all tables and creates appropriate policies for Clerk authentication
-- Run this in your Supabase SQL Editor to fix the RLS security issues
...
-- 2. Drop any existing policies to start fresh (only for tables that exist)
...
-- 15. Create RLS policies for advisor_chats table (if it exists)
...
-- 16. Create RLS policies for advisor_messages table (if it exists)
...
SELECT 'RLS ENABLED FOR ALL TABLES - PRODUCTION READY!' AS status;
